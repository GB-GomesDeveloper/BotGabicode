...
exports.run = async(client, message, args) => {
    ...
    if(!message.member.permissions.has('MANAGE_CHANNELS')){
        ...
        .setDescription(`👮 Staffs, o Membro ${message.author} Usou um codigo (${config.prefix}unmute) que ele não tem permição!`)
    ...
    if(!members){
        ...
        .addField(`👨‍💻 Exemplo ➔`, `${config.prefix}unmute @indivíduo`)
        .setColor('PURPLE')
        .setTimestamp()
        .setThumbnail(message.author.displayAvatarURL({dynamic: true}))
        .setFooter(`${message.guild.name} - Alguns direitos reservados ®`, message.guild.iconURL({dynamic:true}))
    ...
    members.roles.remove(cargo);
    
    let embedaviso = new Discord.MessageEmbed()
    .setAuthor(message.author.username, message.author.displayAvatarURL({dynamic: true}))
    .setTitle('Membro Mutado!')
    .setDescription(`${message.author}, **O membro ${members} foi Desmutado.**`)
    .addField(`👮‍♂️ Para mutar  ➔`, `Digite: ${config.prefix}mute`)
    .setTimestamp()
